...
\DIFdelbegin \DIFdel{K jednomu přístupovému }\DIFdelend \DIFaddbegin \DIFadd{Nejvhodnější řešení rozšíření tohoto přístupového }\DIFaddend systému \DIFdelbegin \DIFdel{je }\DIFdelend \DIFaddbegin \DIFadd{o senzorovou síť je připojením WSN gatewaye ke kontrolnímu panelu přes RS485 síť stejně jako CKP zařízení z důvodu jednoduché instalace. Jednu nebo i více gatewayí je pak }\DIFaddend možné připojit \DIFdelbegin \DIFdel{několik WSN gatewayí k libovolným kontrolním panelům }\DIFdelend \DIFaddbegin \DIFadd{do vybraných sítí RS485 přístupového systému }\DIFaddend v budově pro dosažení požadovaného pokrytí \DIFdelbegin \DIFdel{WSN}\DIFdelend \DIFaddbegin \DIFadd{senzorové sítě}\DIFaddend .
\DIFaddbegin \DIFadd{Názorná blokové schéma přístupového systému firmy IMA s implementací WSN gatewaye je zobrazeno v obrázku \ref{fig:ACS architecture IMA with geteway}.
}\DIFaddend 
...
, }\DIFaddend RS485 transceiver \cite{rs485tr} \DIFdelbegin \DIFdel{, viz.
obr. \ref{fig:gatewayBlockDiagram}.
}\DIFdelend \DIFaddbegin \DIFadd{a vývojový kit NUCLEO-L073RZ \mbox{%DIFAUXCMD
\cite{nucleoST} }\hspace{0pt}%DIFAUXCMD
jako řídící jednotka.
}\DIFaddend 
