\chapter{Realizace \DIFdelbegin \DIFdel{zařízení}\DIFdelend \DIFaddbegin \DIFadd{WSN}\DIFaddend }
Tato kapitola popisuje kompletní návrh řešení rozšíření komernčně používaného sytému o senzorovou síť.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\DIFdelbegin \DIFdel{Implementace }\DIFdelend \DIFaddbegin \DIFadd{Návrh integrace }\DIFaddend WSN \DIFaddbegin \DIFadd{gatewaye }\DIFaddend do \DIFaddbegin \DIFadd{stávajcí infrastruktury }\DIFaddend přístupového systému}
...
\DIFdelbegin \DIFdel{Jedná se o kolizní protokol, kde všechna zařízení se řídí pravidlem LBT (Listen Before Talk), kolize jsou detekovány mechanismem kontroly integrity použitý v každé zprávě. V případě že zařízení přijme poškozený příkaz, vyžádá jeho opakování.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Přístupový systém firmy IMA funguje tak, že CKP zařízení získává seznam všech platných karet (ID Credentials) ze serveru řízení přístupu a na základě tohoto seznamu pak CKP zařízení provádí odpovídající akci po přiložení karty ke čtečce. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \subsection{\DIFdel{Sestavení HW k vytvoření prototypu gatewaye k testování}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{Protokol CKP v síti RS485 pro komunikaci s kontrolním panelem}}
\DIFadd{CKP protokol v sítí RS485 pro komunikaci s kontrolním panelem je kolizní protokol, kde všechna zařízení se řídí pravidlem LBT (Listen Before Talk), kolize jsou detekovány mechanismem kontroly integrity použitý v každém příkazu. V případě že zařízení přijme poškozený příkaz, vyžádá jeho opakování. 
...
\DIFadd{Je-li CKP zařízení spuštěno, má status offline a periodicky odesílá příkaz s informaci o jeho statusu na kontrolní panel, který po obdržení tohoto příkazu předá seznam adres koncových zařízení senzorové sítě sekvení několika příkazů a pak ho přepde do statusu online. V této chvíli již gateway přeposílá data z koncových zařízení senzorové síťe na kontrolní panel.
}



%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\DIFadd{Sestavení HW k vytvoření prototypu gatewaye k testování}}
\DIFaddend Pro otestování navrženého řešení rozšíření přístupového systému o senzorovou síť je sestavena gateway z vývojového kitu NUCLEO-L073RZ \cite{nucleoST}, transceiveru Dragino LoRa Shield \cite{RFM95w} a RS485 transceiver \cite{rs485tr}, viz. obr. \ref{fig:gatewayBlockDiagram}.
...
K vytvoření prototypu byl použit, přotože splňuje všechny požadované parametry, je levný, dostatečně výkonný a jednoduchý pro vývoj. Kit obsahuje programátor ST-link pro nahrávání a debugování programu, tudíž není potřeba externí programátor během vývoje \DIFdelbegin \DIFdel{aplikace}\DIFdelend \DIFaddbegin \DIFadd{FW}\DIFaddend .
...
\DIFdelbegin \subsubsection{\DIFdel{Zapojení}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Zapojení}}
\DIFaddend LoRa shield \cite{draginoWiki} je nasazen přímo na vývojový kit Nukleo. Kit neobsahuje ISCP konektor, který je součástí pinoutu Arduino UNO a LoRa shield má SPI piny MISO a MOSI přivedeny právě na tento konektor. Musí být tedy propojeny externě viz obrázek \ref{fig:03}. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \subsection{\DIFdel{Použité vývojové nástroje k vytvoření FW}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{Použité vývojové nástroje k vytvoření FW}}
\DIFaddend Výrobce použitého mikrokontroléru STMicroelectronics poskytuje grafický nástroj STM32CubeMX \cite{stm32cubemx} ke konfiguraci mikrokontrolérů STM32 a generování odpovídajícího kôdu v programovacím jazyce C.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \subsection{\DIFdel{Vývoj FW}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{Návrh FW gatewaye}}
\DIFaddend Pro tento vybraný případ neni využit RTOS (Real Time Operating System), jelikož zde neni potřeba řešení více úloh souběžně. 
...
\DIFdelbegin \DIFdel{Vyvinutá aplikace je rozdělena }\DIFdelend %DIF >  \subsection{Rozdělení programu}
\DIFaddbegin \DIFadd{Vyvinutý program je rozdělen }\DIFaddend na nékolik nezávislých podprogramů viz tabulka \ref{table:sw_div}. 
...
\begin{table}[!h]
    ...
    \end{tabular}
    \end{ctucolortab}
    \caption{Rozdělení \DIFdelbeginFL \DIFdelFL{aplikace }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{programu }\DIFaddendFL na nezávislé podprogramy}
    \label{table:sw_div}
\end{table}
...
%DIF >  \subsection{Ukládání informací do non-volatile paměti}
...
%DIF >  \subsection{Běh programu}
Po spuštění \DIFdelbegin \DIFdel{aplikace }\DIFdelend \DIFaddbegin \DIFadd{programu }\DIFaddend je nejprve inicializován mikrokontrolér, hodinový signál, GPIO, rozhranní, timery a IWDG (Independent Watchdog). Systémový hodinový signál je nakonfigurová na maximální hodnotu, což je 32 MHz. Po inicializaci mikrokontroléru je z non-volatile paměti EEPROM vyčtena a vypsána v logu aktuální konfigururace gatewaye a následuje inicializace LoRa transceiveru RFM95w dle načtené konfigurace a nakonec je vstoupeno do hlavní smyčky běhu programu, kde jsou dokola nepřetržitě zpracovávány cyklické buffery a fronty tvořeny linked listy. 

Jsou zde použity 2 timery, TIM2 pro restartování procesoru a TIM3 pro resetování IWDG.
Resetování procesoru po daném intervalu defaultně není použito, ale je implementováno a v případě potřeby je možné využít.IWDG je resetováván timerem tudiž tvoří pojistku že resetuje procesor v případě že by \DIFdelbegin \DIFdel{aplikace spadla}\DIFdelend \DIFaddbegin \DIFadd{se program zhroutil}\DIFaddend .

Komunikace s transceiverem RS485 přes LPUART1 je je implementována pro vysílání v blokujícím módu a přijímání přes interrupt, kde přijatý byte je zapsán do cyklického bufferu o velikosti, který je zpracováván v hlavní smyčce běhu \DIFdelbegin \DIFdel{aplikace}\DIFdelend \DIFaddbegin \DIFadd{programu}\DIFaddend . 
...
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \section{\DIFdel{Implementace komunkičního protokolu v síti IMA\_RS485 pro komunikaci s control panelem}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{todo
%DIF <  V tomto projektu je komunikace protokolu IMA\_RS485 naprogramována v souborech rs485\_protocol.h a rs485\_protocol.c. 
%DIF <  Jedná se o kolizní protokol v síti, kde je připojen jedno zařízení typu master, a jeden nebo více zařízení v typu slave.
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
%DIF <       Baud rate              & 9600           \\ \hline
%DIF <       Data bits              & 8                 \\ \hline
%DIF <       Parity                 & none              \\ \hline
%DIF <       Stop bits              & 1                 \\ \hline
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <      \end{tabular}
%DIF <      \caption{Fyzické vlastnosti IMA\_RS485 sítě}
%DIF <      \label{table:3}
%DIF <  \end{table}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
%DIF <  \newpage
%DIF <  \subsection{Syntaxe příkazů}
%DIF <  Komunikace v síti probíhá formou příkazů, které mají specifikovanou syntaxi v tabulce \ref{table:syntaxePrikazu}.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \begin{table}[!h]
%DIF <      \centering
%DIF <  \begin{tabular}{ |c|| p{1.5cm} | p{1.5cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | }
%DIF <   \hline
%DIF <   popis      & adresa příjemce & adresa odesílatele & typ příkazu & délka dat & data & crc\\ \hline
%DIF <   počet bytů & 1               & 1   & 1     & 2     & délka dat     & 1 \\ 
%DIF <   \hline
%DIF <  \end{tabular}
%DIF <      \caption{Syntaxe příkazu pro komunikaci v síti IMA\_RS485}
%DIF <      \label{table:syntaxePrikazu}
%DIF <  \end{table}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Typy příkazu jsou zadefinované konstanty s předponou CKP\_CMD\_ v souboru ./Inc/rs485\_protocol.h.
%DIF <  Příkazy odeslané zařízením typu master obsahují navíc synchronizační byte na začátku 0xAA.
%DIF <  CRC je pro kontrolu XOR přes všechny předchozí byty v celém příkazu kromě synchronizačního bytu.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \subsection{Adresace zařízení v síti}
%DIF <  Každé zařízení na této sběrnici má svoji adresu, která mu je nastavena externě. Zařízení typu master má adresu  0xFF, adresa pro všechny (broadcast) je 0x00 a zařízení v této sítí můžou mít adresu libovolnou (krom těchto dvou) nesmí zde však být připojena 2 zařízení s nastavenou stejnou adresou.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \subsection{Statusy}
%DIF <  Zařízení typu slave má dva možné statusy v síti IMA\_RS485, offline a online. Zařízení typu slave má povoleno odesílat příkaz "průchod" pouze má-li status online.  
%DIF <  Zařízení typu slave odesílá příkaz obsahující informaci o jeho statusu periodicky s typem příkazu 0x10 a jedním bytem dat označujícím status. 
%DIF <  Pro status online je tento byte 0x00 a pro status offline 0xEE. 
%DIF <  Tento příkaz je odesílán s intervalem 10 s, pokud zařízení má status offline a s intervalem 30 s, pokud má zařízení status online.
%DIF <  Status zařízení mění pouze zařízení typu master odesláním příkazu s typem 0x41 pro přepnutí na status online a 0x42 pro přepnutí na status offline.
%DIF <  Zařízení typu slave svůj status přepne samo pouze v případě, že má status online a zařízení typu master přestane odpovídat na příkaz průchod, jak je popsáno v sekci \ref{sec:Odesílání dat z koncových zařízení}.
%DIF <  Je-li zařízení spuštěno, je ve stavu offline a jelikož nemá povoleno odesílat příkaz "průchod", přijatá data z koncových zařízení jsou zahazována. 
%DIF <  Zařízení typu slave pouze odpovídá na příkazy od zařízení typu master a čeká na příkaz od zařízení typu master k přepnutí na status online.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \subsection{Přidávání LoRaWAN zařízení do systému}
%DIF <  Pokud zařízení typu master přijme příkaz od zařízení typu slave oznamující že je ve stavu offline, nejprve tomuto zařízení pošle seznam LoRaWAN adres všech známých koncových zařízení a následně toto zařízení přepne do stavu online.
%DIF <  Přijímání seznamu adres je realizováno sekvencí příkazů typu 0x8F. 
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Níže v tabulce \ref{table:2} je příklad sekvence příkazů odesílaných mezi zařízením typu masterem a zařízením typu slavem během předávání seznamu LoRaWAN device address koncových zařízení, kde zařízení typu slave má adresu 0x10 a zařízení typu master standardně 0xFF. Jak již bylo řečeno, příkazy od zařízení typu master lze jednoduše odlišit tím, že vždy začínají bytem 0xAA.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \begin{table}[!h]
%DIF <      \begin{tabular}{ |l|p{10cm}| }
%DIF <      \hline
%DIF <      příkaz      &  data    \\ \hline \hline
%DIF <      master: start      &  AA 10 FF 8F 02 00 00 00 62    \\ \hline
%DIF <      slave: ACK        &  FF 10 06 02 00 8F 00 64    \\ \hline
%DIF <      master: data     &  AA 10 FF 8F 21 00 01 B1 C4 12 00 00 00 00 00 B2 C4 12 00 00 00 00 00 B3 C4 12 00 00 00 00 00 B4 C4 12 00 00 00 00 00 44 \\ \hline
%DIF <      slave: ACK      &  FF 10 06 02 00 8F 01 65   \\ \hline
%DIF <      master: data     &  AA 10 FF 8F 19 00 02 B5 C4 12 00 00 00 00 00 B6 C4 12 00 00 00 00 00 F6 1F 01 26 00 00 00 00 B6 \\ \hline
%DIF <      slave: ACK      &   FF 10 06 02 00 8F 02 66   \\ \hline
%DIF <      master: konec   &   AA 10 FF 8F 04 00 03 FF 2A 57 E5   \\ \hline
%DIF <      slave: ACK      &   FF 10 06 02 00 8F 03 67  \\ \hline
%DIF <      \end{tabular}
%DIF <      \caption{Příklad sekvence příkazů odesílaných mezi zařízením typu master a zařízením typu slave během předávání seznamu LoRaWAN device address koncových zařízení}
%DIF <      \label{table:2}
%DIF <  \end{table}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  LoraWAN protokol používá 4-bytové adresy koncových zařízení.
%DIF <  Adresy předávány touto sekvencí jsou dlouhé 8-bytové. První 4 byty je tedy LoRaWAN device address, pátý byte je typ zařízení a zbylé 3 byty jsou nevyužity, jejich použití je možné v případě změn či rozšiřování vlastností systému. 
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  První Byte dat je counter paketu začínající od nuly, který označuje číslo odeslaného paketu v sekvenci. Na každý tento paket v sekvenci zařízení typu slave odpovídá ACK příkaz, který se liší od obyčejného ACK příkazu tím, že v datech paketu navíc obsahuje counter pakety v sekvenci.
%DIF <  První příkaz této sekvence má délku dat 2 byty, které mají hodnotu 0x00 přičemž první je counter.
%DIF <  Další příkazy hned za counter bytem obsahují několik osmibytových adres, jejichž počet je různý.
%DIF <  Příkaz ukončující tuto sekvenci příkazů má délku 4 byty, což je tedy counter, 0xFF a 2 byty CRC přes všechny odeslané adresy (nepodstatné, tudíž ho nepoužívám).
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \subsection{Odesílání dat z koncových zařízení} 
%DIF <  \label{sec:Odesílání dat z koncových zařízení}
%DIF <  Jak je popsáno v sekci \ref{sec:Implementace IoT do přístupového systému firmy IMA}, tudíž data z koncových zařízení jsou odesílána příkazem "průchod", jehož typ je 0x10 a kapacita na data z koncového zařízení je pouze 6 B. 
%DIF <  První byte dat označuje typ průchodu, byl zvolen konstantní byte 0xD0. Dále následuje LoRaWAN adresa koncového zařízení od kterého byl paket přijat. Dále následují 4 byty dat ze senzoru, další 2 byty signalizující čas průchodu, což v tomto projektu není použito a tyto dva byty mají vždy hodnotu 0xFF. A nakonec jsou další 2 byty obsahující data ze senzoru.
%DIF <  Příklad příkazu: FF 1F 10 0D 00 D0 F6 1F 01 29 AD 0A 5A 27 FF FF DE 09 E1.
%DIF <  Data příkazu jsou níže rozepsána v tabulce \ref{table:prikladprikazpruchod}.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \begin{table}[!h]
%DIF <      \centering
%DIF <  \begin{tabular}{ | p{1.5cm} | p{3cm} | p{2.5cm} | p{1.3cm} | p{1.3cm} |  }
%DIF <   \hline
%DIF <   typ průchodu & LoRaWAN device address & data (4B)     & cas   & data (2B) \\ \hline
%DIF <   D0           & F6 1F 01 29            &  AD 0A 5A 27  & FF FF & DE 09     \\ 
%DIF <   \hline
%DIF <  \end{tabular}
%DIF <      \caption{Příklad dat příkazu "průchod" odeslaného z gatewaye na zarizeni typu master, obsahující data z koncových zařízení}
%DIF <      \label{table:prikladprikazpruchod}
%DIF <  \end{table}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Zařízení typu master na příkaz "průchod" odpovídá příkazem ACK. Zařízení typu slave na tuto odpověď čeká standardně 3 sekundy, ale tento parametr je nastavitelný. Pokud v tomto timeoutu zařízení typu master neodpoví, zařízení typu slave příkaz "průchod" zopakuje přičemž změní typ příkazu na 0x20. Pokud zařízení typu master ani na třetí opakování neodpoví ACK, zařízení typu slave se přepne do stavu offline a vymaže frontu příkazů "průchod" k odeslání.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \subsection{Potvrzení}
%DIF <  Zařízení typu slave odpovídá na každý příkaz od zařízení typu master ACK. Typ příkazu ACK je 0x06 a data příkazu obsahují jeden byte signalizující typ příkazu na který je právě odpovídáno potvrzením.
%DIF <  Zařízení typu master odpovídá ACK se stejným typem příkazu 0x06, ale s žádnými daty příkazu.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \subsection{Dotaz na příznaky}
%DIF <  Zařízení typu master se může zeptat s jak dlouhými adresami zařízení typu slave pracuje s typem příkazu 0x49. Zařízení typu slave na to odpovídá ACK s tím, že v datech příkazu je navíc byte 0x04. Zařízení typu master pak počítá s tím, že zařízení typu slave pracuje se 64-bit adresami (ve skutečnosti ale používá 32-bitové a zbylé 4 byty v příkazu průchod jsou pro data z koncového zařízení).
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\DIFdel{Komunikace přes sériovou linku USB}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend %DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFaddbegin \subsection{\DIFadd{Komunikace přes sériovou linku USB}}
\DIFaddend \label{Komunikace přes sériovou linku}
...
Při komunikaci jsou data \DIFdelbegin \DIFdel{standardně }\DIFdelend oddělována bytem CR (carriage return) 0x0D, ale je akceptována i sekvence CR LF (Line Feed), tedy 0x0D 0x0A \DIFaddbegin \DIFadd{v hexadecimálním tvaru}\DIFaddend . 
...
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \subsection{\DIFdel{Konfigurace gatewaye}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend %DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFaddbegin \section{\DIFadd{Konfigurace gatewaye}}
\DIFaddend \label{Konfigurace gatewaye}
...
\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \subsection{\DIFdel{Podpora koncových zařízení}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{Podpora koncových zařízení}}
\DIFaddend Jak již bylo zmíněno v sekci \label{Implementace WSN do přístupového systému}, z důvodu datového omezení protokolu sítě RS485 jsou LoRaWAN pakety koncových zařízení dekódovány v gatewayi a z datového obsahu (payloadu) vypočítány konečné hodnoty dle dokumentace daného LoRaWAN zařízení a přes síť RS485 na kontrolní panel jsou odeslány pouze vybraná data. Gateway má pro každé koncové zařízení uloženou 4 byty dlouhou adresu a 1 byte typ zařízení.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \subsection{\DIFdel{Přidávání koncových zařízení ze serveru řízení přístupu}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{Přidávání koncových zařízení ze serveru řízení přístupu}}
\DIFaddend Přidávání koncových zařízení senzorové sítě se provádí z uživatelského rozhranní serveru řízení přístupu stejně jako přidávání platných RFID karet, s délkou UID 8 B.
...
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF >  \section{Implementace komunkičního protokolu v síti IMA\_RS485 pro komunikaci s control panelem}
%DIF >  V tomto projektu je komunikace protokolu IMA\_RS485 naprogramována v souborech rs485\_protocol.h a rs485\_protocol.c. 
%DIF >  Jedná se o kolizní protokol v síti, kde je připojen jedno zařízení typu master, a jeden nebo více zařízení v typu slave.
\DIFaddbegin 
