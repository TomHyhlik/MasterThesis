...
LoRaWAN protokol je zabezpečen šifrováním AES-128, a to zabezpečení aplikační pro nečitelnst přenášených dat a síťové pro zabránění útočníkům opakovat již přenesené pakety nebo odesílat falešné pakety. Jsou zde tedy dva AES-128 šifrovací klíče, applikační klič AppSKey (Application Session Key) a síťový klíč NwKSKey (Network Session Key) \cite{lorawan_specification}.
...
Po spuštění programu je nejprve inicializován mikrokontrolér, hodinový signál, GPIO, rozhraní, timery a IWDG (Independent Watchdog). Systémový hodinový signál je nakonfigurován na maximální hodnotu, což je 32 MHz. Po inicializaci mikrokontroléru je z non-volatile paměti EEPROM vyčtena a vypsána v logu aktuální konfigururace gatewaye a následuje inicializace LoRa transceiveru RFM95w dle načtené konfigurace a nakonec je vstoupeno do hlavní smyčky běhu programu, kde jsou zpracovávány cyklické buffery přijatých dat přes USB a RS485 a fronty přijatých LoRaWAN paketů a příkazů k odeslání na kontrolní panel přes síť RS485. 
...
Komunikace s LoRa transceiverem RFM95W přes rozhraní SPI1 je implementována v \DIFdelbegin \DIFdel{blokujícím }\DIFdelend \DIFaddbegin \DIFadd{blokujcím }\DIFaddend módu zápisem a čtením registrů. 
...
% Po dobu kopírování souboru FW na vývojový kit bliká na kitu  červená-zelená.
% Pro uvedení Gatewaye do provozu je nutné se připojit k zařízení přes USB a nastavit všechny parametry viz sekce \ref{Konfigurace gatewaye}.  
...
% s připojením dle instrukcí v sekci \nameref{Komunikace přes sériovou linku}.
