...
\DIFaddbegin \DIFadd{Vývojový kit NUCLEO-L073RZ je vhodný pro vývoj kvůli jeho parametrům viz tabulka \ref{tab:mcuFeatures}, pinoutu kompatibilním s Arduino UNO, dokumentaci a ceně (\$13 USD\mbox{%DIFAUXCMD
...
\centering
\footnotesize
\caption{\DIFaddFL{Vlastnosti procesoru STM32L073RZ \mbox{%DIFAUXCMD
\cite{nucleoST}}\hspace{0pt}%DIFAUXCMD
}}
\begin{ctucolortab}
...
\DIFadd{Pro komunikaci s LoRa transceiverem je použito SPI1, pro komunikaci přes USB je použito USART2 a pro připojení RS485 transceiveru je použito USART1.
}
...
\begin{table}[h]
    ...
    \begin{ctucolortab}
     ...
     \DIFaddFL{Periférie          }& \DIFaddFL{Název pinu }& \DIFaddFL{Pin procesoru           }\\ \hline \hline
    ...
    \end{tabular}
    \end{ctucolortab}
    \caption{\DIFaddFL{Pinout připojení externích periférií k procesoru}}
    \label{table:3}
\end{table}


\DIFaddend %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFaddbegin \subsection{\DIFadd{Vývoj FW pro prototyp gatewaye}}

\DIFadd{todo
}

\subsubsection{\DIFadd{Použité vývojové nástroje}}
\DIFadd{todo
}



\subsubsection{\DIFadd{Nahrávání programu do procesoru a debugování}}
\DIFadd{todo
}
...
\DIFaddbegin \newpage
\DIFaddend \begin{lstlisting}[style=log]
