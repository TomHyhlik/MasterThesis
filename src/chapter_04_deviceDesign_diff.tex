...
\subsubsection{Architecture of Access Control Systems}
...
\subsubsection{Wireless Sensor Network Design}
...
\begin{figure*}[!h]
\centering
\includegraphics[width=140mm]{ACS_IoT_extension_21}
% \subfigure[Plot3.]{\label{fig2b} \includegraphics[width=80mm]{fig2b.png}}\\
% \subfigure[Mesh.]{\label{fig2c} \includegraphics[width=80mm]{fig2c.png}}
...
% \begin{figure}[!h]
% \centering
% \includegraphics[width=80mm]{ACS_IoT_extension_21}
% \caption{Architektura přístupového systému firmy IMA}
% \label{fig:ACS architecture IMA with geteway}
% \end{figure}

The Access Control System operates by obtaining a CKP list of valid cards (ID Credentials) from the Access Control Server.
...
The most important requirements for the WSN Gateway are simplicity, low cost and the ability to use third-party nodes.
LoRa with the standardized network protocol LoRaWAN is chosen as wireless communication technology, but only in single-channel mode as used in development projects \cite{Analysis of Propagation Link for Remote Weather}, so it is not fully LoRaWAN compliant.
Single-channel Gateway communicates only on one channel and data rate at a time, therefore all devices in WSN are configured for one frequency channel and data rate.
The advantage of a single-channel Gateway is about ten times cheaper transceiver and lower CPU performance requirements than standard LoRaWAN Gateway transceiver.
Third-party LoRaWAN nodes are fully compatible with any LoRaWAN Gateway, so it can be deployed into this system, but it needs to be configured to communicate at the specified frequency channel and data rate.
