...
Pro každý podporovaný typ koncového zařízení musí být implementováno zpracování payloadu ve FW \DIFaddbegin \DIFadd{(FW) }\DIFaddend gatewaye. Pozdější přidání typů koncových zařízení tedy vyžaduje update FW gatewaye.
Gateway tedy má uloženou informaci o typu koncového zařízení společně s jeho LoRaWAN device address.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %DIF > % todo, cast teto vet uz je v predchozi kapitole
LoRaWAN protokol je zabezpečen šifrováním AES-128 na dva způsoby, a to zabezpečení aplikační pro nečitelnst přenášených dat a síťové pro zabránění \DIFdelbegin \DIFdel{pro zabránění }\DIFdelend útočníkům opakovat již přenesené packety nebo odesílat falešné packety. Jsou zde tedy dva AES-128 šifrovací klíče, applikační klič AppSKey (Application Session Key) a síťový klíč NwKSKey (Network Session Key).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\DIFdelbegin \DIFdel{Stavba }\DIFdelend \DIFaddbegin \DIFadd{Sestavení HW k vytvoření prototypu }\DIFaddend gatewaye \DIFaddbegin \DIFadd{k testování}\DIFaddend }
Pro otestování navrženého řešení rozšíření přístupového systému o senzorovou síť je sestavena gateway z vývojového kitu NUCLEO-L073RZ \cite{nucleoST}, transceiveru Dragino LoRa Shield \cite{RFM95w} a RS485 transceiver \cite{rs485tr}, \DIFdelbegin \DIFdel{Fig. }\DIFdelend \DIFaddbegin \DIFadd{viz. obr. }\DIFaddend \ref{fig:gatewayBlockDiagram}.

\DIFdelbegin %DIFDELCMD < \begin{figure*}[!ht]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.9\textwidth]{5patro}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Location of sensor nodes and CKP devices on the university floor}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:CorridorFloorPlan}
...
%DIF <  The NUCLEO-L073RZ development board with STM32L073RZ microcontroller is suitable for the development purposes because of its parameters, Tab. \ref{tab:mcuFeatures}, price and available documentation.
%DIF < It is  was used for the prototype, because it has all required parameters, it is cheap, powerful enough and easily prototypable.
%DIF <  The price of the devlopment board is \$13 USD\cite{nucleoST}.
%DIF <  LoRa transceiver board RFM95w with SX1276 chip integrated into Dragino LoRa Shield has the same pinout as the NUCLEO-L073RZ development board. RS485 transceiver as an RS485/UART interface enables communication with Control Panel.
\DIFaddbegin \DIFadd{Vývojový kit NUCLEO-L073RZ s integrovaným mikrokontrolérem STM32L073RZ je vhodný pro vývoj kvůli jeho parametrům viz tabulka \ref{tab:mcuFeatures}, pinoutu kompatibilním s Arduino UNO, dokumentaci a ceně (\$13 USD\mbox{%DIFAUXCMD
...
LoRa transceiver RFM95w je PCB (Printed Circuit Board) s čipem SX1276. Pro tento prototyp je použit integrovaný v Dragino LoRa Shield \mbox{%DIFAUXCMD
...
%DIF < RS485 transceiver SparkFun Breakout is used for the communication with the Control Panel. It converts interfaces UART to RS485, Fig. \ref{fig:gatewayBlockDiagram}.
\DIFaddbegin \begin{table}[h]
\centering
\footnotesize
\caption{\DIFaddFL{Vlastnosti mikrokontroléru STM32L073RZ \mbox{%DIFAUXCMD
\cite{nucleoST}}\hspace{0pt}%DIFAUXCMD
}}
\begin{ctucolortab}
\begin{tabular}{|l|p{3.5cm}|}
\hline
\DIFaddFL{Microcontroller architecture }& \DIFaddFL{ARM Cortex-M0+ 32-bit RISC }\\ \hline
\DIFaddFL{Internal flash memory }& \DIFaddFL{192 KB }\\ \hline
\DIFaddFL{Internal SRAM memory }& \DIFaddFL{20 kB }\\ \hline
\DIFaddFL{Internal EEPROM memory }& \DIFaddFL{6 kB }\\ \hline
\DIFaddFL{CPU frequency }& \DIFaddFL{up to 32 MHz }\\ \hline
\DIFaddFL{Interfaces }& \DIFaddFL{2X SPI, 3x I2C, 4x UART, LIN }\\ \hline
\end{tabular}
\end{ctucolortab}
...
%DIF <  \centering
%DIF <  \footnotesize
%DIF <  \caption{The STM32L073RZ features \cite{nucleoST}}
%DIF <  \begin{tabular}{|l|p{3.5cm}|}
%DIF <  \hline
%DIF <  Microcontroller architecture & ARM Cortex-M0+ 32-bit RISC \\ \hline
%DIF <  Internal flash memory & 192 KB \\ \hline
%DIF <  Internal SRAM memory & 20 kB \\ \hline
%DIF <  Internal EEPROM memory & 6 kB \\ \hline
%DIF <  CPU frequency & up to 32 MHz \\ \hline
%DIF <  Interfaces & 2X SPI, 3x I2C, 4x UART, LIN \\ \hline
%DIF <  \end{tabular}
%DIF <  \label{tab:mcuFeatures}
%DIF <  \end{table}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \section{Výběr komponent}
%DIF <  \subsection{Microcontroller}
%DIF <  Pro toto zařízení je zvolen mikrocontroller STM32L073RZ se zaměřením na nízkou spotřebu, jelikož je levný, má dostačující vlastnosti a je dostupný ve formě vývojového kitu NUCLEO-L073RZ který byl použit pro  vývoj zařízení. Mezi hlavní vlastnosti patří \cite{nucleoST}:
%DIF <  \begin{itemize}    
%DIF <      \item {Architektura ARM Cortex-M0+ 32-bit RISC}
%DIF <      \item{Interní Flash paměť 192 KB}
%DIF <      \item{Interní SRAM paměť 20 KB}
%DIF <      \item{Interní EEPROM paměť 6 KB}
%DIF <      \item {Až 32 MHz CPU}
%DIF <      \item {2X SPI, 3x I2C, 4x USART, LIN, ADC}
%DIF <  \end{itemize}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend % Pořizovací cena kitu přímo na stránce výrobce www.st.com je \$13 \cite{nucleoST} \cite{nucleoMbed}.
% \begin{figure}[!h]
%     \centering
...
%DIF <  \subsection{LoRa transceiver}
%DIF <  Lora transceiver čip doposud vyrábí pouze Semtech, pro použití v Evropském pásmu je určen typ SX1276.
%DIF <  V tomto návrhu je použita deska RFM95w od firmy HopeRF s integrovaným čipem SX1276 \cite{RFM95w}.
%DIF <  Pro vývoj zařízení byl využit tento transciever v tzv. Dragino LoRa Shield \cite{draginoWiki}, který má stejně jako použitý vývojový kit, pinout kompatibilní s Arduino UNO. Pořizovací cena samotného transceiveru RFM95w je okolo \$7, cena Dragino Shieldu se pohybuje okolo \$22 na ebay.
\DIFdelbegin %DIFDELCMD < 
...
\DIFaddbegin \subsubsection{\DIFadd{Zapojení}}
\DIFadd{LoRa shield \mbox{%DIFAUXCMD
\cite{draginoWiki} }\hspace{0pt}%DIFAUXCMD
je nasazen přímo na vývojový kit Nukleo. Kit neobsahuje ISCP konektor, který je součástí pinoutu Arduino UNO a LoRa shield má SPI piny MISO a MOSI přivedeny právě na tento konektor. Musí být tedy propojeny externě viz obrázek \ref{fig:03}. 
%DIF >  Jumpery na Dragino LoRa shieldu musí také být stejně jako v obrázku.
}

\begin{figure}[!h]
    \centering
    \includegraphics[width=1\textwidth]{foto01}
    \caption{\DIFaddFL{foto zapojení}}
    \label{fig:03}
\end{figure}

%DIF >  TODO: popsat jak funguje na nucleu uart <-> USB prevodnik.
\DIFadd{Pro komunikaci s LoRa transceiverem je použito SPI1, pro komunikaci přes USB je použito USART2 a pro připojení RS485 transceiveru je použito USART1. Připojení periférií k GPIO (General Purpose Input Output) mikrokontroléru je znázorněno v tabulce \ref{table:Pinout připojení periférií k mikrokontroléru}.
}
...
\begin{table}[h]
    ...
    \begin{ctucolortab}
     ...
     \DIFaddFL{Periférie          }& \DIFaddFL{Název pinu }& \DIFaddFL{GPIO mikrokontroléru           }\\ \hline \hline

                        & \DIFaddFL{CS    }&  \DIFaddFL{PB6             }\\
                        & \DIFaddFL{CLK   }&  \DIFaddFL{PA5        }\\
    \DIFaddFL{LoRa transceiver    }& \DIFaddFL{MISO  }&  \DIFaddFL{PA6     }\\
                        & \DIFaddFL{MOSI  }&  \DIFaddFL{PA7        }\\
                        & \DIFaddFL{RST   }& \DIFaddFL{PC7          }\\
                        & \DIFaddFL{DIO0  }& \DIFaddFL{PA10         }\\
                        \hline

                        & \DIFaddFL{RX  }&   \DIFaddFL{PC1            }\\
    \DIFaddFL{RS485 transceiver   }& \DIFaddFL{TX  }&   \DIFaddFL{PC0       }\\
                        & \DIFaddFL{RTS  }&  \DIFaddFL{PB1      }\\     \hline

    \DIFaddFL{USB konektor        }& \DIFaddFL{RX    }& \DIFaddFL{PA3    }\\
                        & \DIFaddFL{TX    }& \DIFaddFL{PA2   }\\          \hline

    \end{tabular}
    \end{ctucolortab}
    \caption{\DIFaddFL{Pinout připojení periférií k mikrokontroléru}}
    \label{table:Pinout připojení periférií k mikrokontroléru}
\end{table}


\DIFaddend %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFaddbegin \subsection{\DIFadd{Použité vývojové nástroje}}
\DIFadd{Výrobce použitého mikrokontroléru STMicroelectronics poskytuje grafický nástroj STM32CubeMX \mbox{%DIFAUXCMD
\cite{stm32cubemx} }\hspace{0pt}%DIFAUXCMD
ke konfiguraci mikrokontrolérů STM32 a generování odpovídajícího kôdu v programovacím jazyce C.
...
\DIFadd{Kód byl vyvíjen v textovém editoru Visual Studio Code \mbox{%DIFAUXCMD
\cite{vscode}}\hspace{0pt}%DIFAUXCMD
, kompilován kompilátorem arm-none-eabi-gcc \mbox{%DIFAUXCMD
\cite{arm-none-eabi-gcc} }\hspace{0pt}%DIFAUXCMD
pomocí nástroje GNU make \mbox{%DIFAUXCMD
\cite{makefile}}\hspace{0pt}%DIFAUXCMD
, výsledný binární soubor byl nahráván do procesoru pomocí nástroje st-flash \mbox{%DIFAUXCMD
\cite{st-flash} }\hspace{0pt}%DIFAUXCMD
a debugován pomocí Visual Studio Code rozšíření Cortex-Debug \mbox{%DIFAUXCMD
\cite{cortex-debug}}\hspace{0pt}%DIFAUXCMD
.
}

\subsubsection{\DIFadd{Nahrávání programu do mikrokontroléru a debugování}}
\DIFadd{todo
}
...
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFaddend % \section{Implementace komunkičního protokolu v síti IMA\_RS485 pro komunikaci s control panelem}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Komunikace přes sériovou linku \DIFaddbegin \DIFadd{USB}\DIFaddend }
...
\DIFdelbegin %DIFDELCMD < \begin{lstlisting}[style=log]
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{lstlisting}[style=CStyle]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \section{\DIFdel{Přidávání koncových zařízení ze serveru řízení přístupu}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Přidávání koncových zařízení ze serveru řízení přístupu}}
...
%DIFDELCMD < %%%
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF <  \section{Zapojení}
%DIF <  LoRa shield \cite{draginoWiki} je nasazen přímo na vývojový kit Nukleo. Kit neobsahuje ISCP konektor, který je součástí pinoutu Arduino UNO a LoRa shield má SPI piny MISO a MOSI přivedeny právě na tento konektor. Musí být tedy propojeny externě viz obrázek \ref{fig:03}. Jumpery na Dragino LoRa shieldu musí také být stejně jako v obrázku.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \begin{figure}[!h]
%DIF <      \centering
%DIF <      \includegraphics[width=1\textwidth]{foto01}
%DIF <      \caption{foto zapojení}
%DIF <      \label{fig:03}
%DIF <  \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Pro komunikaci s LoRa transceiverem je tedy použito SPI1, pro komunikaci přes USB je použito USART2 a pro komunikaci přes RS485 je použito UART1.
%DIFDELCMD < 
...
%DIFDELCMD < %%%
%DIF <       Periférie          & Název pinu & Pin procesoru           \\ \hline \hline
     %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <                          & RX  &   PC1            \\
%DIF <      RS485 transceiver   & TX  &   PC0       \\
%DIF <                          & RTS  &  PB1      \\     \hline
%DIFDELCMD < 
...
%DIFDELCMD < %%%
%DIF <      \end{tabular}
%DIF <      \caption{Pinout připojení externích periférií k procesoru}
%DIF <      \label{table:3}
%DIF <  \end{table}
\DIFdelend 
