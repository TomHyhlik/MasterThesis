\chapter{Návrh senzorové sítě}
\DIFaddbegin \label{Návrh WSN}
\DIFaddend Tato kapitola popisuje návrh řešení rozšíření přístupového sytému o senzorovou síť.
...
\DIFdelbegin \DIFdel{Rozšíření tohoto Přístupového systému o WSN je vytvořeno připojením WSN gatewaye ke kontrolnímu panelu přes RS485 síť stejně jako CKP zařízení. Názorná blokové schéma přístupového systému firmy IMA s implementací WSN gatewaye je zobrazeno v obrázku \ref{fig:ACS architecture IMA with geteway}.
}\DIFdelend V budovách, kde již je tento přístupový systém nainstalován je obvykle více kontrolních panelů, vždy jeden kontrolní panel pro několik dveří s CKP zařízeními propojenými sítí RS485. 
\DIFdelbegin \DIFdel{K jednomu přístupovému }\DIFdelend \DIFaddbegin \DIFadd{Nejvhodnější řešení rozšíření tohoto přístupového }\DIFaddend systému \DIFdelbegin \DIFdel{je }\DIFdelend \DIFaddbegin \DIFadd{o senzorovou síť je připojením WSN gatewaye ke kontrolnímu panelu přes RS485 síť stejně jako CKP zařízení z důvodu jednoduché instalace. Jednu nebo i více gatewayí je pak }\DIFaddend možné připojit \DIFdelbegin \DIFdel{několik WSN gatewayí k libovolným kontrolním panelům }\DIFdelend \DIFaddbegin \DIFadd{do vybraných sítí RS485 přístupového systému }\DIFaddend v budově pro dosažení požadovaného pokrytí \DIFdelbegin \DIFdel{WSN}\DIFdelend \DIFaddbegin \DIFadd{senzorové sítě.
Názorná blokové schéma přístupového systému firmy IMA s implementací WSN gatewaye je zobrazeno v obrázku \ref{fig:ACS architecture IMA with geteway}}\DIFaddend .

\begin{figure}[!h]
\centering
\includegraphics[width=1\textwidth]{ACS_IoT_extension_21}
%DIF >  \caption{IMA access control system architecture with WSN extension}
\caption{\DIFaddbeginFL \DIFaddFL{Architektura přístupového systému firmy }\DIFaddendFL IMA \DIFdelbeginFL \DIFdelFL{access control system architecture with WSN extension}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{s rozšířením o gateway senzorové sítě}\DIFaddendFL }
\label{fig:ACS architecture IMA with geteway}
\end{figure}

WSN gateway tedy musí podporovat CKP protokol v sítí RS485 navržen firmou IMA.
Přístupový systém firmy IMA funguje tak, že CKP zařízení získává seznam všech platných \DIFdelbegin \DIFdel{karet (ID Credentials) }\DIFdelend \DIFaddbegin \DIFadd{identifikačních čísel karet }\DIFaddend ze serveru řízení přístupu a na základě tohoto seznamu pak CKP zařízení provádí odpovídající akci po přiložení karty ke čtečce. 
...
Tento problém je řešen \DIFdelbegin \DIFdel{tak}\DIFdelend \DIFaddbegin \DIFadd{ta}\DIFaddend , že LoRaWAN pakety z koncových zařízení jsou dešifrovány přímo v gatewayi a konečné hodnoty senzorů jsou vypočítány z payloadu zařízení na základě dokumentace poskytnuté výrobcem \cite{RHF1S001 pdf}. Vybrané hodnoty jsou odeslány na server řízení přístupu skrze kontrolní panel. 
...
Pro každý podporovaný typ koncového zařízení musí být implementováno zpracování datového obsahu (payloadu) ve FW \DIFdelbegin \DIFdel{(FW) }\DIFdelend gatewaye. Pozdější přidání typů koncových zařízení tedy vyžaduje update FW gatewaye.
Gateway tedy má uloženou informaci o typu koncového zařízení společně s jeho LoRaWAN \DIFdelbegin \DIFdel{device address}\DIFdelend \DIFaddbegin \DIFadd{adresa zařízení (device address)}\DIFaddend .
%% todo, cast teto vet uz je v predchozi kapitole
LoRaWAN protokol je zabezpečen šifrováním AES-128\DIFdelbegin \DIFdel{na dva způsoby}\DIFdelend , a to zabezpečení aplikační pro nečitelnst přenášených dat a síťové pro zabránění útočníkům opakovat již přenesené pakety nebo odesílat falešné pakety. Jsou zde tedy dva AES-128 šifrovací klíče, applikační klič AppSKey (Application Session Key) a síťový klíč NwKSKey (Network Session Key).
...
Zařízení typu slave má dva možné statusy, offline a online, \DIFdelbegin \DIFdel{periodický }\DIFdelend \DIFaddbegin \DIFadd{periodicky }\DIFaddend odesílá příkaz s informací o jeho aktuálním statusu, má povoleno odesílat příkaz s daty z koncového zařízení senzorové sítě "průchod" pouze má-li status online a změnu statusu daného zařízení typu slave provádí zařízení typu master. 
...
Pro otestování navrženého řešení \DIFdelbegin \DIFdel{rozšíření přístupového systému o senzorovou síť je sestavena gateway z vývojového kitu NUCLEO-L073RZ \mbox{%DIFAUXCMD
\cite{nucleoST}}\hspace{0pt}%DIFAUXCMD
, transceiveru }\DIFdelend \DIFaddbegin \DIFadd{je vytvořena gateway z embedded komponent, tj. LoRa transceiveru pro komunikaci s koncovými zařízeními senzorové sítě, RS485 transceiveru pro komunikaci s kontrolním panelem v síti RS485 a mikrokontrolér jako řídící jednotku viz. blokový diagram v obrázku \ref{fig:gatewayBlockDiagram}.
Konkrétní komponenty jsou: }\DIFaddend Dragino LoRa Shield \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{RFM95w} }\hspace{0pt}%DIFAUXCMD
a }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{draginoWiki}}\hspace{0pt}%DIFAUXCMD
, }\DIFaddend RS485 transceiver \cite{rs485tr} \DIFdelbegin \DIFdel{, viz. obr. \ref{fig:gatewayBlockDiagram}}\DIFdelend \DIFaddbegin \DIFadd{a vývojový kit NUCLEO-L073RZ \mbox{%DIFAUXCMD
\cite{nucleoST} }\hspace{0pt}%DIFAUXCMD
jako řídící jednotka}\DIFaddend .

\begin{figure}[!h]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\textwidth]{LoRaWAN_gw_RS485_blockDiagram_2}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\textwidth]{LoRaWAN_gw_RS485_blockDiagram_3}
    \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Gateway block }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Blokový }\DIFaddendFL diagram \DIFaddbeginFL \DIFaddFL{navržené gatewaye senzorové sítě}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{RFM95w \mbox{%DIFAUXCMD
\cite{RFM95w}}\hspace{0pt}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Dragino LoRa Shield \mbox{%DIFAUXCMD
\cite{draginoWiki}}\hspace{0pt}%DIFAUXCMD
}\DIFaddendFL , RS485 transceiver \cite{rs485tr}, NUCLEO-L073RZ \cite{nucleoST}}
...
\DIFdelbegin \DIFdel{LoRa transceiver RFM95w je PCB (Printed Circuit Board) s čipem SX1276. Pro tento prototyp je použit integrovaný v Dragino LoRa Shield \mbox{%DIFAUXCMD
\cite{draginoWiki}}\hspace{0pt}%DIFAUXCMD
, který má kompatibilní pinout s Arduino UNO, tedy i s použitým vývojovým kitem NUCLEO-L073RZ.
...
\DIFaddbegin \DIFadd{Dragino LoRa Shield \mbox{%DIFAUXCMD
\cite{draginoWiki} }\hspace{0pt}%DIFAUXCMD
má kompatibilní pinout s Arduino UNO, tedy i s použitým vývojovým kitem NUCLEO-L073RZ a obsahuje LoRa transceiver RFM95w \mbox{%DIFAUXCMD
\cite{RFM95w}}\hspace{0pt}%DIFAUXCMD
, tedy PCB (Printed Circuit Board) s čipem SX1276. 
}

\DIFadd{RS485 transceiver konvertuje rozhraní RS485/UART pro umožnění komunikace s kontrolním panelem přes síť RS485.
}

\DIFaddend % Pořizovací cena kitu přímo na stránce výrobce www.st.com je \$13 \cite{nucleoST} \cite{nucleoMbed}.
% \begin{figure}[!h]
%     \centering
...
\begin{figure}[!h]
    ...
    \caption{\DIFdelbeginFL \DIFdelFL{foto zapojení}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Foto sestavené gatewaye}\DIFaddendFL }
    \label{fig:03}
\end{figure}
...
Pro \DIFdelbegin \DIFdel{tento vybraný případ návrhu FW }\DIFdelend \DIFaddbegin \DIFadd{návrh FW gatewaye }\DIFaddend není \DIFdelbegin \DIFdel{využit }\DIFdelend \DIFaddbegin \DIFadd{použit }\DIFaddend RTOS (Real Time Operating System), jelikož zde není potřeba řešení více úloh souběžně. 
...
\begin{table}[!h]
    ...
    LoRa\_sensors    & Knihovna zahrnující \DIFdelbeginFL \DIFdelFL{dekó dování }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{dekódování }\DIFaddendFL payloadu podporovaných koncových zařízení a zapsání výsledných hodnot do bufferu, který je pak odeslán na kontrolní panel přes síť RS485       \\ \hline
...
Každé LoRaWan zařízení v síti má v paměti uložené LoRaWAN \DIFdelbegin \DIFdel{device address }\DIFdelend \DIFaddbegin \DIFadd{adresu zařízení }\DIFaddend (4 byty), typ zařízení (1 byte) a další 3 byty jsou rezervovány. 
...
Po spuštění programu je nejprve inicializován mikrokontrolér, hodinový signál, GPIO, rozhraní, timery a IWDG (Independent Watchdog). Systémový hodinový signál je \DIFdelbegin \DIFdel{nakonfigurová }\DIFdelend \DIFaddbegin \DIFadd{nakonfigurován }\DIFaddend na maximální hodnotu, což je 32 MHz. Po inicializaci mikrokontroléru je z non-volatile paměti EEPROM vyčtena a vypsána v logu aktuální konfigururace gatewaye a následuje inicializace LoRa transceiveru RFM95w dle načtené konfigurace a nakonec je vstoupeno do hlavní smyčky běhu programu, kde jsou \DIFdelbegin \DIFdel{dokola nepřetržitě }\DIFdelend zpracovávány cyklické buffery \DIFaddbegin \DIFadd{přijatých dat přes USB a RS485 a fronty přijatých LoRaWAN paketů }\DIFaddend a \DIFdelbegin \DIFdel{fronty tvořeny linked listy}\DIFdelend \DIFaddbegin \DIFadd{příkazů k odeslání na kontrolní panel přes síť RS485}\DIFaddend . 

Jsou zde použity 2 timery, TIM2 pro restartování procesoru a TIM3 pro resetování IWDG.
Resetování procesoru po daném intervalu defaultně není použito, ale je implementováno a v případě potřeby je možné využít. IWDG je resetováván timerem \DIFdelbegin \DIFdel{tudiž }\DIFdelend \DIFaddbegin \DIFadd{tudíž }\DIFaddend tvoří pojistku \DIFaddbegin \DIFadd{tak, }\DIFaddend že resetuje procesor v případě\DIFaddbegin \DIFadd{, }\DIFaddend že by se program zhroutil.

Komunikace s transceiverem RS485 přes LPUART1 je je implementována pro vysílání v \DIFdelbegin \DIFdel{blokujícím }\DIFdelend \DIFaddbegin \DIFadd{blokujcím }\DIFaddend módu a přijímání přes interrupt, kde přijatý byte je zapsán do cyklického bufferu o velikosti, který je zpracováván v hlavní smyčce běhu programu. 
...
\DIFaddbegin \DIFadd{K nahrání FW do gatewaye je využit ST-link na vývojovém kitu NUCLEO-L073RZ. 
...
%DIF >  Po dobu kopírování souboru FW na vývojový kit bliká na kitu  červená-zelená.
%DIF >  Pro uvedení Gatewaye do provozu je nutné se připojit k zařízení přes USB a nastavit všechny parametry viz sekce \ref{Konfigurace gatewaye}.  
}\DIFaddend 
...
Po připojení ke gatewayi přes sériovou linku \DIFdelbegin \DIFdel{dle instrukcí v sekci }%DIFDELCMD < \nameref{Komunikace přes sériovou linku} %%%
\DIFdelend %DIF >  dle instrukcí v sekci \nameref{Komunikace přes sériovou linku} 
je možné kontinuálně snímat \DIFdelbegin \DIFdel{a }\DIFdelend log gatewaye obsahující informace o proběhlých událostech. Je-li gateway v normálním pracovním režimu, loguje informace automaticky přes sériovou linku a komunikace je zde pouze jednosměrná, tedy gateway vysílá a PC přijímá.
Níže je příklad výpisu logu v případě, kdy gateway \DIFdelbegin \DIFdel{přjala }\DIFdelend \DIFaddbegin \DIFadd{přijala }\DIFaddend LoRaWAN paket z koncového zařízení senzorové sítě, zpracovala ho a výslednou informaci odeslala na kontrolní panel přes RS485 síť.
...
Konfigurace gatewaye se provádí obousměrnou komunikací po sériové lince\DIFdelbegin \DIFdel{s připojením dle instrukcí v sekci }%DIFDELCMD < \nameref{Komunikace přes sériovou linku}%%%
\DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{. 
%DIF >  s připojením dle instrukcí v sekci \nameref{Komunikace přes sériovou linku}.
}\DIFaddend Po spuštění je gateway v normálním pracovním režimu, přeposílá tedy pakety ze senzorové sítě přes RS485 síť na kontrolní panel a informace loguje přes sériovou linku.
...
Pro případ, kde typ zařízení je 01 a DevAddr AABBCCDD (little endian) výsledné číslo v hexadecimální podobě je 01DDCCBBAA. Následně se překládá do \DIFdelbegin \DIFdel{decimalni }\DIFdelend \DIFaddbegin \DIFadd{decimální }\DIFaddend podoby, výsledné číslo k zadání do uživatelského rozhraní serveru řízení přístupu je tedy 8016149418.
...
%DIFDELCMD < %%%
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF <  \section{Nahrání programu do MCU}
%DIF <  Výstupem kompilace je soubor s koncovkou .binary, který je nahrán do MCU. K tomuto nahrání není potřeba žádný speciální SW nebo HW.
%DIF <  Stačí kit připojit k PC přes USB, v PC se kit zobrazí jako flash disk. Zkompilovaný program s koncovkou .binary stačí překopírovat na toto zařízení. 
%DIF <  Po dobu kopírování souboru bliká na kitu LED1 červená/zelená. Jakmile kopírování skončí, program na kitu je spuštěn, případně je možné kit resetovat černým tlačítkem reset.
%DIF <  Pro uvedení Gatewaye do provozu je nutné se připojit k zařízení přes USB a nastavit všechny parametry viz sekce \ref{sec:konfigurace}.  
\DIFdelend 
