...
Navržená gateway bere seznam platných karet jako seznam platných koncových zařízení senzorové sítě. Gateway pak zpracovává \DIFdelbegin \DIFdel{packety }\DIFdelend \DIFaddbegin \DIFadd{pakety }\DIFaddend pouze od zařízení z tohoto seznamu a ostatní ignoruje, tudíž v síti RS485 jsou přenášena pouze relevantní data.
...
%DIF < Since the CKP protocol allows to transmit fixed-size command with only 10 bytes of space, the first 4 bytes are LoRaWAN device address of the node from which the packet was received and the 6 remaining bytes are used for the node data.
%DIF > Since the CKP protocol allows to transmit fixed-size command with only 10 bytes of space, the first 4 bytes are LoRaWAN device address of the node from which the paket was received and the 6 remaining bytes are used for the node data.
LoRaWAN protokol používá \DIFdelbegin \DIFdel{packet}\DIFdelend \DIFaddbegin \DIFadd{paket}\DIFaddend , jehož samotná hlavička je o minimální velikosti 13 B. 
Různá LoRaWAN koncová zařízení používají různé velikosti \DIFdelbegin \DIFdel{payloadu}\DIFdelend \DIFaddbegin \DIFadd{datového obsahu (payloadu)}\DIFaddend , takže není možné odesílat celý payload koncového zařízení přes síť RS485.
Tento problěm je řešen ta, že LoRaWAN \DIFdelbegin \DIFdel{packety }\DIFdelend \DIFaddbegin \DIFadd{pakety }\DIFaddend z koncových zařízení jsou dešifrovány přímo v gatewayi a konečné hodnoty senzorů jsou vypočítány z payloadu zařízení na základě dokumentace poskytnuté výrobcem \cite{RHF1S001 pdf}. Vybrané hodnoty jsou odeslány na server řízení přístupu skrze kontrolní panel. 
%DIF <  In summary, when the Gateway receives an encrypted LoRaWAN packet, it looks for whether the Node device address is in the list of known device addresses.
%DIF <  If this is the case, the packet is encoded in the communication protocol of the Gateway and sent to the Control Panel. If there is not enough space in communication protocol of the Gateway, the packet is decrypted and relevant data, i.e., sensor values, are selected.
%DIF >  In summary, when the Gateway receives an encrypted LoRaWAN paket, it looks for whether the Node device address is in the list of known device addresses.
%DIF >  If this is the case, the paket is encoded in the communication protocol of the Gateway and sent to the Control Panel. If there is not enough space in communication protocol of the Gateway, the paket is decrypted and relevant data, i.e., sensor values, are selected.
...
Jak již bylo zmíňeno v předchozí sekci, gateway dešifruje \DIFdelbegin \DIFdel{packety }\DIFdelend \DIFaddbegin \DIFadd{pakety }\DIFaddend z koncových zařízení a na základě typu koncového zařízení vypočítává z \DIFdelbegin \DIFdel{payloadu }\DIFdelend \DIFaddbegin \DIFadd{datového obsahu (payloadu) }\DIFaddend hodnoty senzorů z nichž vybrané odesílá na server řízení přístupu přes kontolní panel a to z důvodu datového omezení protokolu v síti RS485, do které je gateway připojena.
Pro každý podporovaný typ koncového zařízení musí být implementováno zpracování \DIFdelbegin \DIFdel{payloadu }\DIFdelend \DIFaddbegin \DIFadd{datového obsahu (payloadu) }\DIFaddend ve FW (FW) gatewaye. Pozdější přidání typů koncových zařízení tedy vyžaduje update FW gatewaye.
...
LoRaWAN protokol je zabezpečen šifrováním AES-128 na dva způsoby, a to zabezpečení aplikační pro nečitelnst přenášených dat a síťové pro zabránění útočníkům opakovat již přenesené \DIFdelbegin \DIFdel{packety }\DIFdelend \DIFaddbegin \DIFadd{pakety }\DIFaddend nebo odesílat falešné \DIFdelbegin \DIFdel{packety}\DIFdelend \DIFaddbegin \DIFadd{pakety}\DIFaddend . Jsou zde tedy dva AES-128 šifrovací klíče, applikační klič AppSKey (Application Session Key) a síťový klíč NwKSKey (Network Session Key).
...
%DIF <  Pro tento projekt byla vyvinuta knihovna pro dekódování payloadu na základě dokumentů \cite{lwSpec} \cite{lwSecur}.
%DIF >  Pro tento vybraný případ byla vyvinuta knihovna pro dekódování payloadu na základě dokumentů \cite{lwSpec} \cite{lwSecur}.
...
\DIFaddbeginFL 
    ...
    \DIFaddFL{Parametr          }& \DIFaddFL{Informace            }\\ \hline \hline

    \DIFaddendFL Microcontroller architecture & ARM Cortex-M0+ 32-bit RISC \\ \hline
...
Pro komunikaci s LoRa transceiverem je použito SPI1, pro komunikaci přes USB je použito USART2 a pro připojení RS485 transceiveru je použito USART1. Připojení periférií k GPIO (General Purpose Input Output) mikrokontroléru je znázorněno v tabulce \ref{table:Pinout připojení periférií k mikrokontroléru}.
...
\subsection{Vývoj FW}
\DIFaddbegin \DIFadd{Pro tento vybraný případ neni využit RTOS (Real Time Operating System), jelikož zde neni potřeba řešení více úloh souběžně. K vývoji jsou použity drivery distribuovány výrobcem mikrokontroléru, CMSIS (Cortex Microcontroller Software Interface Standard) a HAL (Hardware Abstraction Layer).
}\DIFaddend 

\DIFaddbegin \DIFadd{Vyvinutá aplikace je rozdělena na nékolik nezávislých podprogramů viz tabulka \ref{table:sw_div}. 
}
...
\begin{table}[!h]
    ...
    \begin{ctucolortab}
     ...
     \DIFaddFL{Název podprogramu  }& \DIFaddFL{Popis           }\\ \hline \hline

    \DIFaddFL{LoRa            }&  \DIFaddFL{Knihovna zahrnující komunikaci s transceiverem SX1276 po rozhranní SPI, nainicializovaném v režimu komunikace LoRa    }\\ \hline
    \DIFaddFL{LoRaWAN\_packet  }& \DIFaddFL{Knihovna zahrnující LoRaWAN protokol, tedy dekódování a dešifrování přijatého LoraWAN paketu i vytvoření paketu pro odeslání. Tato knihovna je závislá na knihovnách aes a cmac.  }\\ \hline
    \DIFaddFL{LoRa\_sensors    }& \DIFaddFL{Knihovna zahrnující dekôdování payloadu podporovaných koncových zařízení a zapsání výsledných hodnot do bufferu, který je pak odeslán na kontrolní panel přes síť RS485       }\\ \hline
    \DIFaddFL{rs485\_protocol  }& \DIFaddFL{Podprogram zahrnující síťový protokol v síti RS485         }\\ \hline
    \DIFaddFL{usb             }&  \DIFaddFL{Podprogram zahrnující konfiguraci systému přes USB        }\\ \hline
    \DIFaddFL{eeprom          }&  \DIFaddFL{Knihovna zahrnující práci s non-volatile paměti procesoru EEPROM     }\\ \hline

    \end{tabular}
    \end{ctucolortab}
    \caption{\DIFaddFL{Rozdělení aplikace na nezávislé podprogramy}}
    \label{table:sw_div}
\end{table}

\DIFadd{Každý z těchto podprogramů je implementován ve zdrojovém souboru (source file) s koncovkou .c a hlavičkovém souboru (header file) s koncovkou .h.
Pro vytvoření knihovny zahrnující LoRaWAN protokol jsou použity open source knihovny pro šifrování, tiny-AES128-C \mbox{%DIFAUXCMD
\cite{lib_tiny-AES128-C} }\hspace{0pt}%DIFAUXCMD
pro zašifrování payloadu a openpana \mbox{%DIFAUXCMD
\cite{lib_openpana} }\hspace{0pt}%DIFAUXCMD
pro vytvoření CMAC (Cipher-based Message Authentication Code)
}
...
\begin{table}[!h]
    ...
    \DIFaddFL{buffer\_ring            }& \DIFaddFL{Knihovna implementující cyklický buffer   }\\ \hline
    \DIFaddFL{ByteArray               }& \DIFaddFL{Knihovna zahrnující pomocné funkce pro práci s polem bytů       }\\ \hline
    \DIFaddFL{LinkedList\_ByteArray   }& \DIFaddFL{Knihovna implementující linked list s neomezenou velikostí o datové struktuře pole bytů s ukládáním do části paměti heap    }\\ \hline
    \DIFaddFL{aes                     }& \DIFaddFL{Knihovna implementující šifrování AES-128 \mbox{%DIFAUXCMD
\cite{lib_tiny-AES128-C}                }\hspace{0pt}%DIFAUXCMD
}\\ \hline
    \DIFaddFL{cmac                    }& \DIFaddFL{Knihovna pro výpočet CMAC, převzatý pouze potřebný kód z knihovný \mbox{%DIFAUXCMD
\cite{lib_openpana}       }\hspace{0pt}%DIFAUXCMD
}\\ \hline

    \end{tabular}
    \end{ctucolortab}
    \caption{\DIFaddFL{Dodatečné pomocné podprogramy}}
    \label{table:sw_div}
\end{table}
...
\DIFaddend \subsubsection{Nahrávání programu do mikrokontroléru a debugování}
todo
...
\DIFdelbegin %DIFDELCMD < \begin{lstlisting}[style=CStyle]
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{lstlisting}[style=log]
...
Nejprve jsou vypsány data týkající se LoRaWAN protokolu, zašifrovaný i dešifrovaný \DIFdelbegin \DIFdel{payload }\DIFdelend \DIFaddbegin \DIFadd{datový obsah (payload) }\DIFaddend koncového zařízení, typ zařízení a výsledné hodnoty dekódované z payloadu.
...
Po spuštění je gateway v normálním pracovním režimu, přeposílá tedy \DIFdelbegin \DIFdel{packety }\DIFdelend \DIFaddbegin \DIFadd{pakety }\DIFaddend ze senzorové sítě přes RS485 síť na kontrolní panel a informace loguje přes sériovou linku.
...
Jak již bylo zmíněno v sekci \label{Implementace WSN do přístupového systému}, z důvodu datového omezení protokolu sítě RS485 jsou LoRaWAN \DIFdelbegin \DIFdel{packety }\DIFdelend \DIFaddbegin \DIFadd{pakety }\DIFaddend koncových zařízení dekódovány v gatewayi a z \DIFdelbegin \DIFdel{dat payloadu}\DIFdelend \DIFaddbegin \DIFadd{datového obsahu (payloadu) }\DIFaddend vypočítány konečné hodnoty dle dokumentace daného LoRaWAN zařízení a přes síť RS485 na kontrolní panel jsou odeslány pouze vybraná data. Gateway má pro každé koncové zařízení uloženou 4 byty dlouhou adresu a 1 byte typ zařízení.
