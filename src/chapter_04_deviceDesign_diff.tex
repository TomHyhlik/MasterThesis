...
Rozšíření tohoto \DIFdelbegin \DIFdel{Přístupového }\DIFdelend \DIFaddbegin \DIFadd{přístupového }\DIFaddend systému o WSN je vytvořeno připojením WSN gatewaye ke kontrolnímu panelu přes RS485 síť stejně jako CKP zařízení. Názorná blokové schéma přístupového systému firmy IMA s implementací WSN gatewaye je zobrazeno v obrázku \ref{fig:ACS architecture IMA with geteway}.
...
Konkrétní komponenty jsou: }\DIFaddend Dragino LoRa Shield \cite{RFM95w}\DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend RS485 transceiver \cite{rs485tr} \DIFdelbegin \DIFdel{, viz. obr. \ref{fig:gatewayBlockDiagram}}\DIFdelend \DIFaddbegin \DIFadd{a vývojový kit NUCLEO-L073RZ \mbox{%DIFAUXCMD
\cite{nucleoST} }\hspace{0pt}%DIFAUXCMD
