\chapter{\DIFdelbegin \DIFdel{Realizace }\DIFdelend \DIFaddbegin \DIFadd{Návrh }\DIFaddend WSN}
Tato kapitola popisuje \DIFdelbegin \DIFdel{kompletní }\DIFdelend návrh řešení rozšíření \DIFdelbegin \DIFdel{komernčně používaného }\DIFdelend \DIFaddbegin \DIFadd{přístupového }\DIFaddend sytému o senzorovou síť.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Návrh integrace WSN gatewaye do stávajcí infrastruktury přístupového systému}
...
K vytvoření prototypu byl použit, přotože splňuje všechny požadované parametry, je levný, dostatečně výkonný a jednoduchý pro vývoj. Kit obsahuje programátor ST-link pro nahrávání a debugování programu, tudíž není potřeba externí programátor během vývoje FW.
...
\subsection{Zapojení}
\DIFaddbegin \DIFadd{CKP zařízení daného přístupového systému jsou obvykle napájena napětím 12 V z kontrolního panelu. Použitý kit NUCLEO-L073RZ obsahuje stabilizátor tudíž je možné připojit gateway přímo ke kontrolnímu panelu přes vyvedené svorky GND, +12V, A a B.
}
...
Vyvinutý program je rozdělen na nékolik nezávislých podprogramů viz tabulka \ref{table:sw_div}. 
...
\DIFaddbegin \subsubsection{\DIFadd{Příklad}}
\DIFaddend Pro případ, kde typ zařízení je 01 a DevAddr AABBCCDD (little endian) výsledné číslo v hexadecimální podobě je 01DDCCBBAA. Následně se překládá do decimalni podoby, výsledné číslo k zadání do uživatelského rozhranní serveru řízení přístupu je tedy 8016149418.




%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFaddbegin \section{\DIFadd{Návrh WSN gatewaye verze 2}}
...
Navíc je zde přidán externí stabilizátor, napěťový filtr, přepínačem volitelné impedanční zakončení síťě RS485 a proudové ochrany.
}
...
%DIF <  \subsection{Nahrání programu do MCU}
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
