...
\DIFaddbegin \DIFadd{Tato kapitola porovnává dostupné bezdrátové technologie vhodné pro tento vybraný případ a na závěr vysvětluje která technologie byla nakonec výbrána a z jakých důvodů.
}\\
\DIFaddend Mezi hlavní požadavky na navrženou senzorovou síť \DIFdelbegin \DIFdel{pro tento vybraný případ patří jednoduchá napojitelnost na infrastrukturu jiného systému. Pro tento vybraný případ se konkrétně jedná o infrastrukturu již zavedeného }\DIFdelend \DIFaddbegin \DIFadd{implementovanou do infrastruktury }\DIFaddend přístupového systému \DIFdelbegin \DIFdel{v budovách několika podniků.
}\DIFdelend \DIFaddbegin \DIFadd{patří jednoduchost napojení.
%DIF >  Pro tento vybraný případ se konkrétně jedná o infrastrukturu již zavedeného přístupového systému v budovách několika podniků.
}\DIFaddend Takto nainstalovaná senzorová síť by pak měla umožňovat bezdrátové měření veličin např. teplota, vlhkost, CO\textsubscript{2}, pomocí koncových zařízení sítě, která vydrží několik let napájena z baterie při periodě měření několik minut rozmístěných v budově a jejím okolí.
