...
Tato kapitola porovnává dostupné bezdrátové technologie vhodné pro tento vybraný případ \DIFaddbegin \DIFadd{senzorové sítě }\DIFaddend a na závěr vysvětluje která technologie byla nakonec výbrána a z jakých důvodů.
...
%DIF <  Navržená senzorová síť má být připojena do infrastruktury přístupového systému zavedeného v několika budovách. Jejím účelem je bezdrátové měření veličin např. teplota, vlhkost, CO\textsubscript{2}, pomocí koncových zařízení sítě, která vydrží několik let napájena z baterie při periodě měření několik minut rozmístěných v budově a jejím okolí.
%DIF <  Pro jednoduchost implementace vybraná bezdrátová technologie tedy musí používat pouze bezlicenční pásmo ISM a musí umožňovat implementaci celé sítě bez závislosti na siti třetích stran. 
%DIF <  Dosah by tedy měl být pro pokrytí celé budovy a jejího okolí, ale je zde i možnost napojení více gatewayí rozmístěných po budově k dosažení požadovaného dosahu.
\DIFaddbegin \DIFadd{Navržená senzorová síť má být připojena do infrastruktury přístupového systému, který je již zaveden v několika budovách. Jejím účelem je bezdrátové měření veličin např. teplota, vlhkost, CO\textsubscript{2}, pomocí koncových zařízení sítě, která vydrží několik let napájena z baterie při periodě měření několik minut rozmístěných v budově a jejím okolí.
...
}\DIFaddend Níže jsou vypsány hlavní kritéria pro výběr bezdrátové technologie pro tuto senzorovou síť.

\begin{itemize}
  \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \item \DIFadd{Nízká cena HW
  }\item \DIFadd{Jednoduché přípojení koncových zařízení třetích stran (Third party)
  }\item \DIFadd{Velký počet dostupných koncových zařízení třetích stran na trhu 
  }\item \DIFadd{Jednoduchá implementace
  }\item \DIFadd{Nízká spotřeba energie koncových zařízení
}\DIFaddend \end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Kandidátní bezdrátové technlogie}
\DIFdelbegin \DIFdel{Níže jsou popsány }\DIFdelend \DIFaddbegin \DIFadd{V této sekci jsou rozebrány }\DIFaddend dostupné bezdrátové technologie\DIFdelbegin \DIFdel{vyhovující }\DIFdelend \DIFaddbegin \DIFadd{, které jsou běžně používány pro bezdrátové měření zařízeními napájenými z baterie a vyhovují }\DIFaddend stanoveným kritériím \DIFdelbegin \DIFdel{pro tento vybraný případ}\DIFdelend \DIFaddbegin \DIFadd{vypsaných výše}\DIFaddend .
...
\DIFdelbegin \DIFdel{Z hlediska kriérií pro tento vybraný případ je u této technologie nevýhodou nízký počet zařízení třetích stran dostupných na trhu. 
}\DIFdelend %DIF >  Z hlediska kriérií pro tento vybraný případ je u této technologie nevýhodou nízký počet zařízení třetích stran dostupných na trhu. 
...
%DIF <  V jednokanálovém módu jsou všechna zařízení v síti nakonfigurována na jeden konkrétní kanál a přenosovou rychlost.
%DIF <  Výhoda použití této technolgie v jednokanálovém módu je, že gateway může použít transceiver určen pro koncová zařízení, který je víc jak desetkrát levnější a vyžaduje nižší výkon CPU než transceiver pro gateway, tudíž gateway v jednokanálovém módu je mnohem jednodušší a levnější řešení.
\DIFdelbegin \DIFdel{LoRaWAN }\DIFdelend \DIFaddbegin 

\DIFadd{Technologie IQRF byla zavrhnuta, jelikož je na truhu velmi málo dostupných koncových zařízení třetích stran, které bý bylo možné do sítě připojit. 
...
LoRa má možnost použití v jednokanálovém módu, kde gateway může použít transceiver určen pro }\DIFaddend koncová zařízení\DIFaddbegin \DIFadd{, který je víc jak desetkrát levnější a vyžaduje nižší výkon CPU (Central Processing Unit) než transceiver pro gateway, tudíž gateway v jednokanálovém módu je mnohem jednodušší a levnější řešení.
}\\

\DIFadd{Pro tento vybraný případ senzorové sítě byla tedy vybrána techologie LoRa se standardizovaným síťovým protokolem LoRaWAN v jednokanálovém módu, jako je to používáno ve vývojových projektech \mbox{%DIFAUXCMD
\cite{Analysis of Propagation Link for Remote Weather}}\hspace{0pt}%DIFAUXCMD
, tudíž to není plně LoRaWAN kompatibilní.
V jednokanálovém módu jsou všechna zařízení v síti nakonfigurována na jeden konkrétní kanál a SF.
LoRaWAN koncová zařízení }\DIFaddend třetích stran jsou plně kompatibilní s jakoukoliv LoRaWAN gatewayí pro daný region, tudíž mohou být implementovány do navrženého systému v tomto projektu, ale musí být překonfigurována ke komunikaci na zvoleném kanále a přenosové rychlosti. 
LoRaWAN síť má hvězdicovou topologii při dostatečném dosahu pro pokrytí budovy a jejím okolí, navíc dosah lze ladit parametrem SF. Pro tento vybraný případ \DIFaddbegin \DIFadd{senzorové sítě }\DIFaddend je to vhodné, jelikož pravděpodobně všechna zařízení v síti budou senzory napájeny z baterie, a není předpokládáno, že by byly připojeny zařízení umožňující směrování paketů z důvodu spotřeby energie. 
...
Případné občasně kolize mají za důsledek selhání dorůčení paketu, ale pro \DIFdelbegin \DIFdel{tento }\DIFdelend vybraný případ \DIFaddbegin \DIFadd{měřícího systému }\DIFaddend to není příliš závažný problém.
...
Pro \DIFdelbegin \DIFdel{tento }\DIFdelend vybraný případ \DIFaddbegin \DIFadd{měřícího systému }\DIFaddend se předpokládá přenášení pouhých hodnot senzorů o velikosti několik jednotek až desitek bytů s intervalem několika minut až hodin\DIFdelbegin \DIFdel{, tudíž to pro tento vybraný případ vyhovuje}\DIFdelend .
