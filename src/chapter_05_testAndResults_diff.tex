...
\DIFaddbegin \label{Testování navrženého řešení}
\DIFaddend Tato kapitola zahrnuje testování navržené gatewaye senzorové sítě viz. kapitola \DIFdelbegin \DIFdel{\ref{Návrh WSN}}\DIFdelend \DIFaddbegin \DIFadd{\ref{Návrh senzorové sítě}}\DIFaddend , napojené přes síť RS485 na infrastrukturu přístupového systému firmy IMA v univerzitní budově univerzity ČVUT za reálného provozu s připojenými koncovými zařízeními senzorové sítě, které souběžně odesílají data. Ze zachyceného provozu dat v síti RS485 je odhadnut maximální počet připojených koncových zařízení souběžně odesílající data v senzorové síti při zachování správné funkce přístupového systému.
