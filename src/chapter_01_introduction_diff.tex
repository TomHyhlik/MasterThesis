...
IoT aplikace vyžadující dlouhý dosah jsou vetšinou založené na druhu technologií LPWAN (Low Power Wide Area Network) \cite{A comparative study of LPWAN technologies for large-scale IoTdeployment}. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Many LPWAN wireless communication technologies appeared during its evolution with unlicensed ISM band, e.g., LoRa and SigFoxand licensed band, e.g., }\DIFdelend \DIFaddbegin \DIFadd{Mezi nejpoužívanější z této kategorie, založené na bezlicenčním pásmu ISM, jsou LoRa a SigFox, a založené na lincenčním pásmu jsou NB-IoT (}\DIFaddend NarrowBand-Internet of Things\DIFaddbegin \DIFadd{) a LTE-M }\DIFaddend (\DIFdelbegin \DIFdel{NB-IoT) and }\DIFdelend Long Term Evolution-Machine Type Communication\DIFaddbegin \DIFadd{).
LPWAN technologie mají dosah 10–15 km v oblastech mimo město a 2–5 km v městských oblastech \mbox{%DIFAUXCMD
\cite{Long-Range Communications in Unlicensed Bands}}\hspace{0pt}%DIFAUXCMD
. Topologie LPWAN sítí jsou hvězdice (centralizovaná), hvězdice z hvězdic }\DIFaddend (\DIFdelbegin \DIFdel{LTE-M).
%DIF <  The LPWAN technologies aim to have range up to 10–15 km in rural areas and 2–5 km in urban areas \cite{Long-Range Communications in Unlicensed Bands} and can have one of the following topologies: star (centralized), star of stars (decentralized) and mesh (distributed) \cite{high density LPWAN}.
%DIF <  Very low power consumption should allow sensor nodes a very long battery life, even greater than 10 years.
%DIF <  The low cost of hardware (HW) is achieved by fully integrated transceivers and minimized number of off-chip components \cite{MURS Band for LPWAN Applications}.
}\DIFdelend \DIFaddbegin \DIFadd{decentralizovaná) a mesh (distribuovaná) \mbox{%DIFAUXCMD
\cite{high density LPWAN}}\hspace{0pt}%DIFAUXCMD
.
Nízká spotřeba koncových zařízení senzorových sítí jim umožňuje dlouhou životnost z baterie, i delší než 10 let.
Nízké ceny HW (Hardware) je dosaženo úplnou integrací transceiverů s minimálním množstvím komponentů mimo čip \mbox{%DIFAUXCMD
\cite{MURS Band for LPWAN Applications}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend 
